...
def get_Faces_and_Coordinates_Beta(image, scaleFactor = 1.3, minNeighbors = 5, pil = True, resize = True, model = 'faceNet'):
    coordinates = face_cascade.detectMultiScale(image, scaleFactor, minNeighbors)
    images = [cv2_to_pil(image[y:y+h, x:x+w]) for (x,y,w,h) in coordinates]
    return images, coordinates
